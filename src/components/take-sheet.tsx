    ...
    <DialogPrimitive.Root open={item !== null} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Popup
          className="fixed inset-x-0 bottom-0 z-50 flex max-h-[92dvh] flex-col overflow-y-auto bg-paper px-4 pt-3 pb-[max(16px,env(safe-area-inset-bottom))] shadow-sheet outline-none sheet-enter desk:inset-y-0 desk:right-0 desk:left-auto desk:w-[26rem] desk:max-h-none desk:px-6 desk:pt-4 desk:panel-enter"
          aria-describedby={undefined}
        >
          <div className="flex items-center justify-between">
            <DialogPrimitive.Title className="text-xs font-semibold text-stencil-muted">
              {item ? `Take or receive ${item.name}` : ""}
